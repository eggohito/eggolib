...
public class ProjectileCondition {
	...
	public static boolean condition(SerializableData.Instance data, Pair<DamageSource, Float> damageSourceAndAmount) {
		...
		if (data.isPresent("projectile")) {
			specifiedEntityTypes.add(data.get("projectile"));
		}
		if (data.isPresent("projectiles")) {
			specifiedEntityTypes.addAll(data.get("projectiles"));
		}

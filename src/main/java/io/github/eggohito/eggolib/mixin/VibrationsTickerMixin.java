...
public interface VibrationsTickerMixin {
	...
	private static void eggolib$tryListenWithoutParticles(ServerWorld world, Vibrations.ListenerData listenerData, Vibrations.Callback callback, Operation<Void> original) {
		...
		if (((LinkableListenerData) listenerData).eggolib$getLinkedPower().map(GameEventListenerPower::shouldShowParticle).orElse(true)) {
	...
	private static <T extends ParticleEffect> int eggolib$preventSpawningParticles(ServerWorld instance, T particle, double x, double y, double z, int count, double deltaX, double deltaY, double deltaZ, double speed, Operation<Integer> original, ServerWorld world, Vibrations.ListenerData listenerData, Vibrations.Callback callback) {
		return ((LinkableListenerData) listenerData).eggolib$getLinkedPower().map(GameEventListenerPower::shouldShowParticle).orElse(true) ? original.call(instance, particle, x, y, z, count, deltaX, deltaY, deltaZ, speed) : 0;
	}

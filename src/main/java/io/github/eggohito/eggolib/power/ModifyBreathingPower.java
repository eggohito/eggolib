...
public class ModifyBreathingPower extends PrioritizedPower {
	...
	public ModifyBreathingPower(PowerType<?> powerType, LivingEntity livingEntity, Predicate<CachedBlockPosition> breathableBlockCondition, StatusEffect breathingStatusEffect, List<StatusEffect> breathingStatusEffects, Modifier gainAirModifier, List<Modifier> gainAirModifiers, Integer gainAirInterval, Modifier loseAirModifier, List<Modifier> loseAirModifiers, Integer loseAirInterval, DamageSourceDescription damageSourceDescription, RegistryKey<DamageType> damageType, Modifier damageModifier, List<Modifier> damageModifiers, Integer damageInterval, ParticleEffect particle, boolean ignoreRespiration, int priority) {
		...
		if (breathingStatusEffect != null) {
			this.breathingStatusEffects.add(breathingStatusEffect);
		}
		if (breathingStatusEffects != null) {
			this.breathingStatusEffects.addAll(breathingStatusEffects);
		}

		this.gainAirModifiers = new ArrayList<>();
		if (gainAirModifier != null) {
			this.gainAirModifiers.add(gainAirModifier);
		}
		if (gainAirModifiers != null) {
			this.gainAirModifiers.addAll(gainAirModifiers);
		}

		this.gainAirInterval = gainAirInterval;

		this.loseAirModifiers = new ArrayList<>();
		if (loseAirModifier != null) {
			this.loseAirModifiers.add(loseAirModifier);
		}
		if (loseAirModifiers != null) {
			this.loseAirModifiers.addAll(loseAirModifiers);
		}
		...
		if (damageModifiers != null) {
			this.damageModifiers.addAll(damageModifiers);
		}
	...
	public void onGained() {
		damageSource = MiscUtil.createDamageSource(entity.getDamageSources(), damageSourceDescription, damageType);
	}

	public BreathingInfo getGainBreathInfo() {

		BreathingInfo.Builder breathingInfoBuilder = BreathingInfo.gainingAir();
		if (gainAirInterval != null) {
			breathingInfoBuilder.airDelta(gainAirInterval);
		}
		if (!gainAirModifiers.isEmpty()) {
			breathingInfoBuilder.airPerCycle((int) ModifierUtil.applyModifiers(entity, gainAirModifiers, ((BreathingInfoBuilderAccessor) breathingInfoBuilder).getAirPerCycle()));
		}
	...
	public BreathingInfo getLoseBreathInfo() {
		...
		if (loseAirInterval != null) {
			breathingInfoBuilder.airDelta(loseAirInterval);
		}
		if (damageSource != null) {
			breathingInfoBuilder.damageSource(damageSource);
		}
		if (damageInterval != null) {
			breathingInfoBuilder.damageAt(damageInterval);
		}
		if (ignoreRespiration) {
			breathingInfoBuilder.ignoreRespiration();
		}

		breathingInfoBuilder.particleEffect(particle);

		if (!loseAirModifiers.isEmpty()) {
			breathingInfoBuilder.airPerCycle((int) ModifierUtil.applyModifiers(entity, loseAirModifiers, ((BreathingInfoBuilderAccessor) breathingInfoBuilder).getAirPerCycle()));
		}
		if (!damageModifiers.isEmpty()) {
			breathingInfoBuilder.damagePerCycle((float) ModifierUtil.applyModifiers(entity, damageModifiers, ((BreathingInfoBuilderAccessor) breathingInfoBuilder).getDamagePerCycle()));
		}
	...
	public static TypedActionResult<Optional<BreathingInfo>> integrateCallback(LivingEntity livingEntity) {
		...
		if (hpmbp.canBreatheIn(blockPos)) {
			return TypedActionResult.success(Optional.of(hpmbp.getGainBreathInfo()));
		}

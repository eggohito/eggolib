...
public class EggolibTeam extends AbstractTeam {
	...
	public boolean isEqual(@Nullable AbstractTeam team) {
		...
		if (fieldStates.getOrDefault(name, false) && name.equals(team.getName())) {
			i++;
		}
		if (fieldStates.getOrDefault(friendlyFire, false) && friendlyFire.equals(team.isFriendlyFireAllowed())) {
			i++;
		}
		if (fieldStates.getOrDefault(showFriendlyInvisibles, false) && showFriendlyInvisibles.equals(team.shouldShowFriendlyInvisibles())) {
			i++;
		}
		if (fieldStates.getOrDefault(collisionRule, false) && collisionRule.equals(team.getCollisionRule())) {
			i++;
		}
		if (fieldStates.getOrDefault(nametagVisibilityRule, false) && nametagVisibilityRule.equals(team.getNameTagVisibilityRule())) {
			i++;
		}
		if (fieldStates.getOrDefault(deathMessageVisibilityRule, false) && deathMessageVisibilityRule.equals(team.getDeathMessageVisibilityRule())) {
			i++;
		}
